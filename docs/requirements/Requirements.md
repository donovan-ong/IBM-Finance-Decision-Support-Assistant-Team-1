...
| Version | Date | Change |
|---|---|---|
| v1 | Original draft | Initial requirements captured, including FR-02 (Db2 ingestion) and FR-03 (parameterized queries for database access). |
| v2 | 26 Sep 2026 | Status changed to **Approved**. FR-02 (ingest from IBM Db2) and FR-03 (parameterized queries / prepared statements for database queries) **removed** — CSV is now the sole data source for this semester. NFR-05 (database credentials) removed, as no database connection remains in scope. Basis: the Db2 Lite plan on IBM Cloud requires manual re-extension every 30 days and a credit card just to provision — an ongoing maintenance risk with no following semester to recover in if reactivation is missed. CSV alone already satisfies the six core calculations and FR-06 traceability, so FR-01 fully covers data ingestion without Db2. This is a **descope, not a deferral**: there is no future semester planned in which to reintroduce FR-02. |

*Reference: `Data-Source-Decision-CSV-vs-Db2.docx` (Research folder, Team SharePoint) — full pros/cons of CSV-only vs CSV + Db2, the Db2 Lite plan limits that drove the call, and the recommendation with reasoning.*
...
## 1. Overview
An AI-powered conversational assistant that streamlines financial data analysis and decision-making. It consolidates data from multiple sources, understands user questions in natural language, and delivers clear, easy-to-understand answers along with an explanation of how each result was derived. When a response involves a financial calculation, the assistant outlines the calculation method and steps followed, provides supporting sources, and links results to relevant business rules and market context — giving financial analysts full traceability back to the source of every figure and result.
...
| ID | Requirement |
|---|---|
| FR-01 | The system shall ingest structured financial data supplied as CSV files and shall handle common file-format variations, including delimiters and text encoding, without requiring the user to manually clean or reformat the file. CSV is the sole data source for this semester (see Version / Change Log, v2). |
...
*FR-02 (ingest from IBM Db2) and FR-03 (parameterized queries / prepared statements for database queries) have been removed. See Version / Change Log, v2, for the reasoning.*
...
### 4.4 Business Rules and Market Context

| ID | Requirement |
|---|---|
| FR-12 | The system shall retrieve and link relevant business rules or market context to financial results, drawn from an approved, team-maintained reference source, with a source citation. |
| FR-13 | The system shall clearly distinguish, in its response, between the calculated financial result, cited business rules, and cited market context. |
...
*NFR-05 (database credentials must never be hard-coded or stored in the project's source repository) has been removed, as no database connection remains in scope. See Version / Change Log, v2.*
...
6. Assistant provides the result, calculation steps, and source traceability, and links the results to relevant business rules and market context.
7. Assistant provides a short analysis summary where applicable.

**Alternative / Exception Flow:** If required data is missing, invalid, ambiguous, or zero where division by zero would occur, the assistant does not perform the calculation. Instead, it explains the issue and identifies the missing or invalid value or source.

## 7. Scope

### In Scope (this term)
The following activities are included within this project's Scope:
- Support for at least 5 core financial calculations (minimum project requirement); the team has selected 6 to implement: Dollar Variance, Percentage Variance, Gross Margin, Period-over-Period Growth, Year-over-Year (YoY) Growth, and Variance Contribution by Category.
- Show the source of each key figure (Traceability) — e.g., `financial_data_2025.csv`, `financial_data_2026.csv`.
- Link financial results to relevant business rules and/or market context.
- Generate a short, clear analysis summary to support human review and decision-making.
- Reduce time spent collecting and organizing financial data.
- Help analysts focus on decision-making instead of manual data gathering.
- Use synthetic (self-generated) financial data for testing, since real data isn't available.
- Define our own synthetic data policy, ensuring that no real company or personal data is used. All data will be fictional but realistic, using plausible values for items such as revenue and expenses.

### Out of Scope (this term)
The following items are excluded from the project's scope:
- Processing real or confidential data: synthetic data will be used instead, since real financial data isn't available or appropriate for this project.
- Support for autonomous or multi-agent systems is out of scope: the assistant will operate as a single tool that requires user input and guidance.
- Integration with other internal IBM tools not listed in the provided brief: to stay within the agreed scope.
- A mobile application: development will focus on a single platform this term.
- Browser testing will be limited to Chrome and Safari: due to the project's timeline.
- Completing financial transactions: the assistant will support financial analysis only and will not execute financial actions.
- Financial questions outside of defined use cases are out of scope to keep the assistant focused and reliable within its intended purpose.
- A relational database connection (IBM Db2): descoped for this semester in favor of CSV-only ingestion (see Version / Change Log, v2).
...
- All business rules/market statements include a usable citation or source link.
